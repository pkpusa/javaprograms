import java.util.*;
import java.io.*;

class StudentData
{
   private int stuID;
   private String stuName;
   private int stuAge;
   //private int paperID;
    private short paperExamID;
	private char paperStream;
	//private String paperDepartment;
	private String paperCourse;
	private String paperName;
	private byte paperTime;
	private boolean paperType;
	private String paperDescription;




   StudentData()
   {
       //Default constructor
       stuID = 100;
       stuName = "New Student";
       stuAge = 18;
   }
   StudentData(int num1, String str, int num2)
   {
       //Parameterized constructor
       stuID = num1;
       stuName = str;
       stuAge = num2;
   }
   //Getter and setter methods
   /*public int getStuID() {
       return stuID;
   }
   public void setStuID(int stuID) {
       this.stuID = stuID;
   }
   public String getStuName() {
       return stuName;
   }
   public void setStuName(String stuName) {
       this.stuName = stuName;
   }
   public int getStuAge() {
       return stuAge;
   }
   public void setStuAge(int stuAge) {
       this.stuAge = stuAge;
   }*/
   public void input()
   {
       System.out.println("Enter Student Name :");
       
   }

   public void display()
   {
    System.out.println("Student Name is: " + stuName);
    System.out.println("Student Age is: " + stuAge);
    System.out.println("Student ID is: " + stuID);

   }
   
}

public class overload
{
    public static void main(String args[])
   {
       System.out.println("Press 1 to See the Hall Ticket Format ");
       System.out.println("Press 2 to Enter Data");
       System.out.println("Press 3 to Generate Hall Ticket");

       switch(ch)
       {
           case 1:
           {
                StudentData myobj = new StudentData();
                myobj.display();
           }

           case 2:
           {
                StudentData 
           }
       }
       //This object creation would call the default constructor
       StudentData myobj = new StudentData();
       //StudentData dispobj = new StudentData();
       myobj.display();

       /*This object creation would call the parameterized
        * constructor StudentData(int, String, int)*/
       StudentData myobj2 = new StudentData(555, "Chaitanya", 25);
       myobj2.display();
       /*
       System.out.println("Student Name is: "+myobj2.getStuName());
       System.out.println("Student Age is: "+myobj2.getStuAge());
       System.out.println("Student ID is: "+myobj2.getStuID());*/ 
  }
}